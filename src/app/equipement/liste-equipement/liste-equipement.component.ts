  ...
  loadEquipement(){
    this.Serviceapibackend.AllEquipement().subscribe((data:any) => {
      ...
      let listfinale = data.map((el:any) => {
        if (typeof el.etatequipements !== 'undefined') {
          let etatactuelle = el.etatequipements.filter((elx:any) => elx.status == "actuelle");
          if (etatactuelle.length > 0) {
...
get sortData() {
  if (this.Equipement && this.Equipement.length) {
